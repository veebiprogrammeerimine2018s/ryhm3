<?php
  ...
  function readAllPrivatePictureThumbsPage($page, $limit){
	...
	$html = "";
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	$stmt = $mysqli->prepare("SELECT filename, alttext FROM vpphotos3 WHERE privacy=? AND userid=? AND deleted IS NULL LIMIT ?,?");
	echo $mysqli->error;
  ...
  function readAllPublicPictureThumbsPage($page, $limit){
	...
	$html = "";
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	$stmt = $mysqli->prepare("SELECT id, filename, alttext FROM vpphotos3 WHERE privacy<=? AND deleted IS NULL LIMIT ?,?");
	echo $mysqli->error;
